...
class AdminClient {
  ...
  async createUniqueGroups(size: number) {
    let groupName = 'group'
  ...
  async getAdminUser() {
    await this.login()
    const [user] = await this.client.users.find({
      username: 'admin',
    })
    return user
  }
  ...
  async deleteClientScope(clientScopeName: string) {
    await this.login()
    const clientScope =
      await this.client.clientScopes.findOneByName({
        name: clientScopeName,
      })
    return await this.client.clientScopes.del({
      id: clientScope?.id!,
    })
  }
  ...
  async patchUserProfile(
    realm: string,
    payload: UserProfileConfig,
  ) {
    await this.login()

    const currentProfile = await this.client.users.getProfile({
      realm,
    })
  ...
  async createIdentityProvider(
    ...
    const identityProviders =
      ...
      []
    const idp = identityProviders.find(
      ({ name }) => name === idpDisplayName,
    )
  ...
  async linkAccountIdentityProvider(
    username: string,
    idpDisplayName: string,
  ) {
    await this.login()
  ...
  async removeAllLocalizationTexts() {
    await this.login()
    const localesWithTexts =
      await this.client.realms.getRealmSpecificLocales({
        realm: this.client.realmName,
      })
    await Promise.all(
