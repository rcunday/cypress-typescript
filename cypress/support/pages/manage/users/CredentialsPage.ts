export default class CredentialsPage {
  ...
  private readonly emptyStatePasswordBtn =
    'no-credentials-empty-action'
  private readonly emptyStateResetBtn =
    'credential-reset-empty-action'
  private readonly resetBtn = 'credentialResetBtn'
  private readonly setPasswordBtn = 'confirm'
  private readonly credentialResetModal =
  ...
  private readonly closeDataDialogBtn =
    '.pf-c-modal-box [aria-label^="Close"]'

  goToCredentialsTab() {
    cy.intercept('/admin/realms/*/users/*/credentials').as(
      'load',
    )
