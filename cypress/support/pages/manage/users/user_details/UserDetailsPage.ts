...
export default class UserDetailsPage extends PageObject {
  ...
  public goToIdentityProviderLinksTab() {
    ...
    cy.intercept(
      '/admin/realms/master/users/*/federated-identity',
    ).as('fedIdentity')
  ...
  fillUserData() {
    ...
    cy.findByTestId(this.firstNameInput).type(
      this.firstNameValue,
    )
  ...
  private getEnabledSwitch(userName: string) {
    return cy.findByTestId(`${userName}-switch`)
  }

  private getEnabledSwitchLabel(userName: string) {
