...
export default class IdentityProviderLinksTab {
  ...
  public typeLinkAccountModalUserId(userId: string) {
    cy.findByTestId(this.linkAccountModalUserIdInput).type(
      userId,
    )
  ...
  public typeLinkAccountModalUsername(username: string) {
    cy.findByTestId(this.linkAccountModalUsernameInput).type(
      username,
    )
  ...
  public assertNoIdentityProvidersLinkedMessageExist(
    exist: boolean,
  ) {
  ...
  public assertNoAvailableIdentityProvidersMessageExist(
    exist: boolean,
  ) {
  ...
  public assertLinkAccountModalTitleEqual(idpName: string) {
    modalUtils.assertModalTitleEqual(
      `Link account to ${idpName}`,
    )
  ...
  public assertUnLinkAccountModalTitleEqual(idpName: string) {
    modalUtils.assertModalTitleEqual(
      `Unlink account from ${idpName}?`,
    )

    return this
  }

  public assertLinkAccountModalIdentityProviderInputEqual(
    idpName: string,
  ) {
    cy.findByTestId(
      this.linkAccountModalIdentityProviderInput,
    ).should('have.value', idpName)
  ...
  public assertNotificationIdentityProviderLinked() {
    masthead.checkNotificationMessage(
      'Identity provider has been linked',
    )
  ...
  public assertLinkedIdentityProvidersItemsEqual(count: number) {
    if (count > 0) {
      ...
      cy.get(this.linkedProvidersSection + ' tbody').should(
        'not.exist',
      )
    }
  ...
  public assertAvailableIdentityProvidersItemsEqual(
    count: number,
  ) {
    if (count > 0) {
      ...
      cy.get(this.availableProvidersSection + ' tbody').should(
        'not.exist',
      )
    }
    return this
  ...
  public assertLinkedIdentityProviderExist(
    idpName: string,
    exist: boolean,
  ) {
    cy.get(this.linkedProvidersSection).should(
  ...
  public assertAvailableIdentityProviderExist(
    idpName: string,
    exist: boolean,
  ) {
    cy.get(this.availableProvidersSection).should(
