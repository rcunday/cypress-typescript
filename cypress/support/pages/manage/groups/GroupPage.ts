...
export default class GroupPage extends PageObject {
  protected createGroupEmptyStateBtn =
    'no-groups-in-this-realm-empty-action'
  ...
  public createGroup(groupName: string, emptyState: boolean) {
    ...
    groupModal
      ...
      .create()
    cy.intercept('POST', '*/admin/realms/master/groups').as(
      'post',
    )
    return this
  }
  ...
  protected search(
    ...
    if (searchValue) {
      ...
      cy.get(searchField + " button[type='submit']").click({
        force: true,
      })
    } else {
  ...
  public goToGroupChildGroupsTab(groupName: string) {
    ...
    cy.intercept('GET', '*/admin/realms/master/groups/*').as(
      'get',
    )
    ...
    if (!destinationGroupName.includes('root')) {
      for (const destination of destinationGroupName) {
        moveGroupModal
          ...
          .assertModalTitleEqual(
            `Move ${groupName} to ${destination}`,
          )
      }
    }
  ...
  public assertNoGroupsInThisRealmEmptyStateMessageExist(
    exist: boolean,
  ) {
