export default class PageObject {
  ...
  private toolbarActionsButton =
    ".pf-c-toolbar button[aria-label='Actions']"
  private breadcrumbItem =
    '.pf-c-breadcrumb .pf-c-breadcrumb__item'

  protected assertExist(
  ...
  protected assertIsDisabled(
    element: Cypress.Chainable<JQuery>,
  ) {
    return this.assertIsEnabled(element, false)
  }

  protected assertHaveText(
    element: Cypress.Chainable<JQuery>,
    text: string,
  ) {
  ...
  protected assertSwitchStateOff(
    element?: Cypress.Chainable<JQuery>,
  ) {
  ...
  protected assertDropdownMenuIsClosed(
    element?: Cypress.Chainable<JQuery>,
  ) {
  ...
  protected clickDropdownMenuItem(
    ...
    ;(
      element ?? cy.get(this.drpDwnMenuItem).contains(itemName)
    ).click()
    return this
  }
  ...
  protected assertDropdownMenuItemIsSelected(
    ...
    this.assertExist(
      element
        .contains(itemName)
        .find(this.selectItemSelectedIcon),
      isSelected,
    )
    return this
  }
  ...
  protected assertDropdownMenuHasItems(
    ...
    for (const item of items) {
      ...
      this.assertExist(
        element.find(this.drpDwnMenuItem).contains(item),
        true,
      )
    }
    return this
  ...
  protected assertDropdownMenuHasLabels(
    ...
    for (const item of items) {
      element = initialElement ?? cy.get(this.drpDwnMenuList)
      this.assertExist(
        element.find(this.formLabel).contains(item),
        true,
      )
    }
    return this
  ...
  protected assertSelectMenuIsClosed(
    element?: Cypress.Chainable<JQuery>,
  ) {
    element = element ?? cy.get(this.selectMenuList)
  ...
  protected assertSelectMenuItemIsSelected(
    ...
    return this.assertDropdownMenuItemIsSelected(
      itemName,
      isSelected,
      element,
    )
  ...
  protected assertSelectMenuHasItems(
    ...
    for (const item of items) {
      ...
      this.assertExist(
        element.find(this.selectMenuItem).contains(item),
        true,
      )
    }
    return this
  ...
  protected removeChipGroupItem(
    ...
    this.getChipGroupItem(groupName, itemName)
      .find('button')
      .click()
    return this
  }
  ...
  protected assertChipGroupExist(
    ...
    this.assertExist(
      cy.contains(this.chipGroup, groupName),
      exist,
    )
    return this
  }
  ...
  protected assertChipItemExist(
    itemName: string,
    exist: boolean,
  ) {
    cy.get(this.chipItem).within(() => {
      cy.contains(itemName).should(
        (exist ? '' : 'not.') + 'exist',
      )
    })
    return this
  ...
  protected assertChipGroupItemExist(
    ...
    this.assertExist(
      this.getChipGroup(groupName).contains(
        this.chipItem,
        itemName,
      ),
      exist,
