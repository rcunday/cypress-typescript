...
export default class TableToolbar extends CommonElements {
  ...
  constructor() {
    ...
    this.searchInput =
      this.parentSelector +
      '.pf-c-text-input-group__text-input:visible'
    this.changeTypeBtn =
      this.parentSelector + '#change-type-dropdown'
    this.nextPageBtn =
      this.parentSelector + 'button[data-action=next]'
    this.previousPageBtn =
      this.parentSelector + 'button[data-action=previous]'
    this.searchTypeDropdownBtn =
      this.parentSelector +
      "[class*='searchtype'] .pf-c-dropdown__toggle"
    this.searchTypeSelectToggleBtn =
      this.parentSelector +
      "[class*='searchtype'] .pf-c-select__toggle"
    this.actionToggleBtn =
      this.dropdownToggleBtn + "[aria-label='Actions']"
  }
  ...
  selectSecondarySearchType(itemName: FilterAssignedType) {
    ...
    cy.get(this.dropdownSelectToggleItem)
      .contains(itemName)
      .click()
    return this
  }
